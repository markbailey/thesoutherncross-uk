...
describe('POST /api/admin/servers', () => {
  ...
  it('returns 403 when no session (no steamid)', async () => {
    ...
    const res = await POST(makeRequest({ name: 'Test', host: '1.2.3.4', port: 27015, protocol: 'source' }));
  ...
  it('returns 201 and creates server + server_status row when admin posts valid body', async () => {
    Object.assign(mockSessionData, { steamid: '76561198051971258' });
    isAdminMock.mockReturnValue(true);

    const res = await POST(
      makeRequest({ name: 'Alpha Server', host: 'game.example.com', port: 27015, protocol: 'source' }),
    );
    ...
    // Verify row was inserted in DB
    const db = getDb({ dbPath: ':memory:' });
    const row = db.prepare('SELECT id, name, host, port, protocol FROM servers WHERE id = ?').get(body.id) as
  ...
  it('returns 400 for missing name', async () => {
    ...
    const res = await POST(makeRequest({ host: '1.2.3.4', port: 27015, protocol: 'source' }));
  ...
  it('returns 400 for invalid port', async () => {
    ...
    const res = await POST(makeRequest({ name: 'Test', host: '1.2.3.4', port: 99999, protocol: 'source' }));
    expect(res.status).toBe(400);
  });
  ...
  it('returns 400 for invalid protocol', async () => {
    ...
    const res = await POST(makeRequest({ name: 'Test', host: '1.2.3.4', port: 27015, protocol: 'bedrock' }));
    expect(res.status).toBe(400);
  });
});
