...
| `Rename-Item` on `wwwroot` fails (lock) | Aborts after the service is stopped and lingering node processes are killed; script attempts `nssm start` to restore the original service. If that also fails, the site stays down. | If the script's restart attempt succeeded, normal operation has resumed — no further action needed. If the site is still down, find the lock holder via `handle.exe`, clear it, then `nssm start TheSouthernCrossUK`. Reboot if the lock cannot be cleared. |
